 ...
 *   4. Spawn the daemon with CHORD_LOCAL_PRIVATE_KEY=<worker pk> and the fake
 *      agent CLI as its agent.
...
const CLIENT_PK = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80" as Hex;
const WORKER_PK = "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d" as Hex;
...
function spawnDetached(
  ...
  child.stdout?.on("data", (b: Buffer) => process.stdout.write(`[${opts.tag}] ${b}`));
  child.stderr?.on("data", (b: Buffer) => process.stderr.write(`[${opts.tag}] ${b}`));
  return child;
}
...
function runOnce(
  cmd: string,
  args: string[],
  opts: { cwd?: string; env?: NodeJS.ProcessEnv; tag: string; timeoutMs?: number },
): Promise<void> {
...
// ─── workflow steps ──────────────────────────────────────────────────────────
async function waitForChain(client: PublicClient, deadlineMs: number): Promise<void> {
  ...
  fail(`hardhat node never responded with chainId ${HARDHAT_CHAIN_ID}: ${describeErr(lastErr)}`);
}

function readDeployedAddress(name: string): Address {
  const file = path.join(HARDHAT_DIR, "deployments", "localhost", `${name}.json`);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (await portInUse(8545)) {
      fail("port 8545 is already in use — kill the running process before running the smoke test (e.g. `lsof -ti tcp:8545 | xargs kill`)");
    }
    ...
    const publicClient = createPublicClient({ chain: HARDHAT_CHAIN, transport: http(RPC_URL) }) as PublicClient;
    ...
    await runOnce(
      "node",
      [YARN_BIN, "workspace", "@chord/hardhat", "deploy", "--network", "localhost", "--reset"],
    ...
    const createHash = await clientWallet.writeContract({
      ...
      args: [
        ...
        0n,
        ["Smoke test milestone — write a result file"],
        [M1],
        [workerAcct.address],
      ],
    });
    ...
    for (const lg of createReceipt.logs) {
      try {
        const decoded = decodeEventLog({ abi: allEscrowAbi, data: lg.data, topics: lg.topics });
    ...
    if (projectId === null) fail("could not find ProjectCreated in createProject receipt");
    log(`✓ project created (id=${projectId.toString()})`);

    // ── 4. spawn the daemon ──────────────────────────────────────────────────
    log("spawning daemon in LOCAL signing mode...");
    ...
    log(`✓ MilestoneAccepted @ block ${acceptedEvent.log.blockNumber} (tx ${acceptedEvent.log.transactionHash})`);

    // ── 6. wait for MilestoneSubmitted ───────────────────────────────────────
    log("watching for MilestoneSubmitted (fake agent runs + daemon submits)...");
    ...
    log(`✓ MilestoneSubmitted note="${submissionNote.slice(0, 80)}${submissionNote.length > 80 ? "..." : ""}"`);
    ...
    await publicClient.waitForTransactionReceipt({ hash: approveMilestoneHash });
    ...
    log(`✓ MilestonePaid amount=${(paidEvent.args.amount as bigint).toString()} autoReleased=${paidEvent.args.autoReleased}`);
    ...
    log(`✓ worker balance increased by ${delta} (${(Number(delta) / 1e6).toFixed(2)} USDC)`);
